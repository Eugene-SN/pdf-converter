...
- **`VLLM_API_KEY`** — обязательный секрет для сервисов перевода и QA. Airflow DAG `quality_assurance` прекращает авто-коррекцию сразу, если ключ отсутствует или пуст, и немедленно помечает задачу как `failed`. Перед развёртыванием обязательно задайте переменную в `.env`, менеджере секретов Kubernetes, HashiCorp Vault или ином механизме конфигурации.
