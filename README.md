...
При старте Flask API выводится предупреждение `Docling не установлен - будет использован fallback режим`, если в окружении отсутствует библиотека [`docling`](https://pypi.org/project/docling-core/).

**Последствия:**

- API продолжит работу, но вместо полноценной конвертации через Docling будут задействованы упрощённые fallback-процессы (PyMuPDF и другие вспомогательные обработчики).
- Fallback-режим не поддерживает извлечение таблиц, формул и сложной структуры на уровне, который предоставляет Docling, поэтому качество Markdown заметно ниже.
- Метрики Docling в Prometheus/Grafana останутся пустыми, что затрудняет мониторинг пайплайна.

**Как устранить:**

1. Убедитесь, что Docker-образ или виртуальное окружение, где запускается сервис `flask-api`, содержит зависимости Docling:
   ```bash
   pip install "docling-core>=2.0" "docling==2.*"
   ```
2. При использовании Docker обновите соответствующий `Dockerfile`/`requirements.txt`, затем пересоберите образ:
   ```bash
   docker compose build flask-api
   docker compose up -d flask-api
   ```
3. После установки библиотек перезапустите сервис. В логах должна появиться строка `Docling доступен: True`, а предупреждение исчезнет.
